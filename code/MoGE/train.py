"""
Mixture of Graph Experts — 5-fold Cross-Validation Training Script.

Best config:
  python train.py --data_dir <path> --gpu 0 --window_size 40 --stride 3 --k_neighbors 20

Performance: 81.05%, ties with EdgeConv v1 stride=3 baseline. No improvement over simple EdgeConv.
"""
import os
import argparse
import numpy as np
...
from model import MoGE_EdgeDGCNN
...
def main():
    ...
    for fold_idx, (train_val_idx, test_idx) in fold_splits:
        ...
        model = MoGE_EdgeDGCNN(
            in_features=in_features, num_nodes=30,
            k=args.k_neighbors, hidden_dim=64,
            num_experts=args.num_experts, nclass=2,
            spatial_dist=spatial_dist).to(device)
        print(f'Model: MoGE_EdgeDGCNN ({sum(p.numel() for p in model.parameters()):,} params, {args.num_experts} experts)')
