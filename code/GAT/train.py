"""
Graph Attention Network — 5-fold Cross-Validation Training Script.

Best config:
  python train.py --data_dir <path> --gpu 0 --window_size 40 --stride 1 --k_neighbors 20 --model gat
  python train.py --data_dir <path> --gpu 0 --model gat --extra_data_dir <depression_path>

Key finding: GAT soft attention underperforms EdgeConv max-pool on noisy EEG (80.35% vs 82.76%).
"""
import os
import argparse
import numpy as np
...
from model import GATDGCNN, GATDGCNN_DANN
...
def parse_args():
    ...
    p.add_argument('--model', type=str, default='gat', choices=['gat', 'gat_dann'])
    p.add_argument('--lambda_dann', type=float, default=0.1)
...
def train_epoch(model, loader, criterion, optimizer, device, is_dann=False, lambda_=0.0, lambda_dann=0.1):
    model.train()
    ...
    for batch in loader:
        ...
        if is_dann:
            ...
            loss = criterion(cls_logits, Y) + lambda_dann * criterion(subj_logits, S)
            output = cls_logits
        else:
            output = model(X)
...
def evaluate(model, loader, criterion, device, is_dann=False, return_data=False):
    ...
    for batch in loader:
        X, Y = batch[0], batch[1]
        X, Y = X.float().to(device), Y.long().to(device)
        output = model(X, lambda_=0) if is_dann else model(X)
...
def main():
    ...
    for fold_idx, (train_val_idx, test_idx) in fold_splits:
        ...
        if is_dann:
            ...
            train_loader = DataLoader(eegDataset(train_x, train_y, torch.cat(train_s_list)),
                                      batch_size=args.batch_size, shuffle=True)
            val_loader = DataLoader(eegDataset(val_x, val_y, torch.cat(val_s_list)),
        ...
        if args.model == 'gat':
            model = GATDGCNN(in_features=in_features, num_nodes=30,
                             k=args.k_neighbors, nclass=2,
                             spatial_dist=spatial_dist).to(device)
        else:
            n_dann = 2 if args.dann_domain else len(main_subjects)
            model = GATDGCNN_DANN(in_features=in_features, num_nodes=30,
                                  k=args.k_neighbors, nclass=2,
                                  num_subjects=n_dann, spatial_dist=spatial_dist).to(device)
        ...
        test_loss, test_acc = evaluate(model, test_loader, criterion, device, is_dann=is_dann)
