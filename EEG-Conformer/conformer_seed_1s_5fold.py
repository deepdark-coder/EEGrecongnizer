...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40):
        ...
        self.eegnet = nn.Sequential(
            nn.Conv2d(1, 8, (1, 125), (1, 1)), #修改数据shape对应(400, 30, 250)
            nn.BatchNorm2d(8),
...
class ExGAN():
    ...
    def get_source_data(self, fold):
        ...
        mat_file = os.path.join(self.data_dir, f'HC{self.nSub:04d}_1s.mat')
        mat_data = scipy.io.loadmat(mat_file)
        self.all_data = mat_data['data']  # (400, 30, 250)
        self.all_label = mat_data['label'].flatten()
        self.train_data = []
        self.train_label = []
        self.test_data = []
        self.test_label = []
        ...
        self.train_data = np.expand_dims(self.train_data, axis=1)  # (320, 1, 30, 250)
        self.test_data = np.expand_dims(self.test_data, axis=1)   # (80, 1, 30, 250)
